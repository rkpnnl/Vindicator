function y = nanvar1(x,w,dim)
%NANVAR1 Variance, ignoring NaNs.
%   Y = NANVAR1(X) returns the sample variance of the values in X, treating
...
%   For N-D arrays, NANVAR1 operates along the first non-singleton dimension
%   of X.
%
%   nanvar1 normalizes Y by N-1 if N>1, where N is the sample size of the 
...
%
%   Y = NANVAR1(X,1) normalizes by N and produces the second moment of the
%   sample about its mean.  NANVAR1(X,0) is the same as NANVAR1(X).
%
%   Y = NANVAR1(X,W) computes the variance using the weight vector W.  The
%   length of W must equal the length of the dimension over which NANVAR1
%   operates, and its non-NaN elements must be nonnegative.  Elements of X
%   corresponding to NaN elements of W are ignored.
%
%   Y = NANVAR1(X,W,DIM) takes the variance along dimension DIM of X.
%
%   See also VAR, nanstd1, nanmean1, nanmedian1, nanmin1, nanmax1, nansum1.
...
if isequal(w,0) || isequal(w,1)
    ...
    x0 = x - repmat(nanmean1(x, dim), tile);
    y = nansum1(abs(x0).^2, dim) ./ denom; % abs guarantees a real result

% Weighted variance
elseif numel(w) ~= sz(dim)
    error('MATLAB:nanvar1:InvalidSizeWgts','The length of W must be compatible with X.');
elseif ~(isvector(w) && all(w(~isnan(w)) >= 0))
    error('MATLAB:nanvar1:InvalidWgts','W must be a vector of nonnegative weights, or a scalar 0 or 1.');
else
    ...
    % Count up non-NaNs.
    n = nansum1(~isnan(x).*w,dim);

    x0 = x - repmat(nansum1(w.*x, dim) ./ n, tile);
    y = nansum1(w .* abs(x0).^2, dim) ./ n; % abs guarantees a real result
end
