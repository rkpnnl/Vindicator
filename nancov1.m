function c = nancov1(x,varargin)
%NANCOV1 Covariance matrix, ignoring NaNs.
%   C = NANCOV1(X), if X is a vector, returns the sample variance of the
%   values in X, treating NaNs as missing values.  For matrices, where
%   each row is an observation and each column a variable, NANCOV1(X) is
%   the covariance matrix computing using rows of X that do not contain
%   any NaN values.  NANCOV1(X,Y), where X and Y are matrices with
%   the same number of elements, is equivalent to NANCOV1([X(:) Y(:)]). 
%   
%   NANCOV1(X) or NANCOV1(X,Y) normalizes by (N-1) if N>1, where N is the
%   number of observations after removing missing values.  This makes
%   nancov1(X) the best unbiased estimate of the covariance matrix if the
%   observations are from a normal distribution. For N=1, COV normalizes
%   by N.
%
%   NANCOV1(X,1) or NANCOV1(X,Y,1) normalizes by N and produces the second
%   moment matrix of the observations about their mean.  NANCOV1(X,Y,0) is
%   the same as NANCOV1(X,Y), and NANCOV1(X,0) is the same as NANCOV1(X).
%
%   C = NANCOV1(...,'pairwise') computes C(I,J) using rows with no NaN
%   values in columns I or J.  The result may not be a positive definite
%   matrix. C = nancov1(...,'complete') is the default, and it omits rows
...
%       c = nancov1(x)          % compute sample covariance
%
%   Class support for inputs X,Y:
%      float: double, single
%
%   See also COV, VAR, nanvar1.

%   Copyright 1984-2005 The MathWorks, Inc. 
%   $Revision: 1.1.6.3 $  $Date: 2005/03/23 20:25:40 $

if nargin<1
   error('stats:nancov1:NotEnoughInputs', 'Not enough input arguments.');
end
...
if numel(varargin)>0
   ...
   if ischar(temp)
      ...
      if isempty(j)
         error('stats:nancov1:InvalidArg','Invalid argument ''%s''.',temp);
      end
      dopairwise = (j==1);
      varargin(end) = [];
   end
...
if numel(varargin)>1
   error('stats:nancov1:TooManyArgs','Two many agruments.');
end

scalarxy = false; % nancov1(scalar,scalar) is an ambigous case
if numel(varargin)>0
   y = varargin{1};
   ...
   if length(x)~=length(y)
      error('stats:nancov1:XYmismatch', 'The lengths of X and Y must match.');
   end
   scalarxy = isscalar(x) && isscalar(y);
   x = [x y];
elseif ndims(x)>2
   error('stats:nancov1:InputDim', 'Inputs must be 2-D.');
end
